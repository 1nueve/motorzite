...
if ( function_exists('register_sidebar') ) {
	...
	if (function_exists('qtrans_getLanguage')) {
		register_sidebar(array('name'=>'qTranslate Language Chooser Widget',
			'before_widget' => "<div class='widget qtranslate'>",
			'after_widget' => '</div>',
			'before_title' => '<h3 class="bar">',
			'after_title' => '</h3 >',
		));
	}

	if (get_option('wp_site') == "Real Estate") {
		if (get_option('wp_search') != "Only built-in search") {
			register_sidebar(array('name'=>'dsIDXpress Search',
	...
	register_sidebar(array('name'=>'Homepage (under search) ',
		'before_widget' => "<div class='widget'>",
		'after_widget' => '</div>',
		'before_title' => '<h3 class="bar">',
		'after_title' => '</h3 >',
	...
	register_sidebar(array('name'=>'Homepage (under slideshow) 1 ',
		'before_widget' => "<div class='widget'>",
		'after_widget' => '</div>',
		'before_title' => '<h3 class="bar">',
		'after_title' => '</h3 >',
	...
	if (get_option('wp_widgetlayout') != "Layout 4") { 
		register_sidebar(array('name'=>'Homepage (under slideshow) 2 ',
			'before_widget' => "<div class='widget'>",
			'after_widget' => '</div>',
			'before_title' => '<h3 class="bar">',
			'after_title' => '</h3 >',
	...
	if (get_option('wp_widgetlayout') == "Layout 1") { 
		register_sidebar(array('name'=>'Homepage (under slideshow) 3 ',
			'before_widget' => "<div class='widget'>",
			'after_widget' => '</div>',
			'before_title' => '<h3 class="bar">',
			'after_title' => '</h3 >',
	...
	register_sidebar(array('name'=>'Left Sidebar (not homepage)',
		'before_widget' => "<div class='widget'>",
		'after_widget' => '</div>',
		'before_title' => '<h3 class="bar">',
		'after_title' => '</h3 >',
	...
	register_sidebar(array('name'=>'Right Sidebar (not homepage)',
		'before_widget' => "<div class='widget'>",
		'after_widget' => '</div>',
		'before_title' => '<h3 class="bar">',
		'after_title' => '</h3 >',
...
class Contact_Widget extends WP_Widget {
    ...
    function Contact_Widget() {
		global $themename;
		$widget_ops = array('classname' => 'contact-widget', 'description' => __( "Quickly add contact info to your sidebar (e.g. address, phone #, email).", "opendoor") );
		$control_ops = array('width' => 400, 'height' => 200);
		$this->WP_Widget('contact', __('Contact Us', 'opendoor'), $widget_ops, $control_ops);
    }

    function widget($args, $instance) {
        extract( $args );
        $title = apply_filters('widget_title', empty($instance['title']) ? __('Contact Info', 'opendoor') : $instance['title'], $instance, $this->id_base);
		$name = $instance['name'];
