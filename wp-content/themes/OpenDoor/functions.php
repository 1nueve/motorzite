...
function my_enqueue() {
  wp_register_script( 'jQuery', get_template_directory_uri() . '/js/jquery-1.8.2.js' ); 
...
{  
    ...
    wp_register_style( 'base', get_template_directory_uri() . '/css/base.css', array(), '', 'screen' );  
	...
	wp_register_style( 'style', get_template_directory_uri() . '/style.css', array(), '', 'screen' );  
...
// ****************************************************************************************************************************
		...
		array_unshift($dealerlocations, "Choose a dealer location:");
		...
		if ($singlelanguage == true) {		
			...
			array_unshift($fueltype, "Choose an item");
		} else {
			$defaultlang = $defaultlangarray[1];
			//make array of all in dropdown from this language
			foreach ($fueltype as $item) {
		...
		if ($singlelanguage == true) {		
			...
			array_unshift($trans, "Choose an item");
		} else {
			$defaultlang = $defaultlangarray[1];
			//make array of all in dropdown from this language
			foreach ($trans as $item) {
		...
		"description" => "If the automobile also fits into another body type category, choose it here.",
		"options" => $bodytype),
		
		"body_type3" => array(
		"name" => "body_type3",
		"type" => "select",
		"title" => "<strong>Body Type (3rd one)</strong>",
		"description" => "If the automobile also fits into yet another body type category, choose it here.",
		"options" => $bodytype),
		...
		"description" => "Enter the vehicle's VIN number. This is required for outputting VIN data from VINquery.com. A VINquery.com account is required"),
		...
		if (get_option('wp_translate') == "Yes") { 
			...
			// ** External VIN handling code - john@dynamicarray.co.uk - 28/06/12
